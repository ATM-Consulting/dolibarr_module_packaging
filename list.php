...
$hookmanager->initHooks(array('packaginglist'));
...
llxHeader('', $langs->trans('packagingList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->packaging->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'packaging t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'packaging_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('packaging', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_packaging', 'GET');
...
$listViewConfig = array(
	...
	,'list' => array(
		'title' => $langs->trans('packagingList')
		,'image' => 'title_generic.png'
		,'picto_precedent' => '<'
		,'picto_suivant' => '>'
		,'noheader' => 0
		,'messageNothing' => $langs->trans('Nopackaging')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => packaging::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
