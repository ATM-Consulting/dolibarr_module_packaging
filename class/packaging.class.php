...
class packaging extends SeedObject
{
    /**
     * Canceled status
     */
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'packagingStatusShortCanceled'
		,self::STATUS_DRAFT => 'packagingStatusShortDraft'
		,self::STATUS_VALIDATED => 'packagingStatusShortValidated'
//		,self::STATUS_REFUSED => 'packagingStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'packagingStatusShortAccepted'
	);

	/** @var string $table_element Table name in SQL */
	public $table_element = 'packaging';

	/** @var string $element Name of the element (tip for better integration in Dolibarr: this value should be the reflection of the class name with ucfirst() function) */
	public $element = 'packaging';
...
//        ),
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->PACKAGING_REF_MASK) ? $conf->global->PACKAGING_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'packaging', 'ref');
...
//            $this->fk_user_valid = $user->id;
    ...
    public function getNomUrl($withpicto = 0, $moreparams = '')
    {
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showpackaging") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;

        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/packaging/card.php', 1).'?id='.$this->id.urlencode($moreparams).$linkclose;
...
//        $picto='packaging@packaging';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('packagingStatusCancel'); $statusLabelShort=$langs->trans('packagingStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('packagingStatusDraft'); $statusLabelShort=$langs->trans('packagingStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('packagingStatusValidated'); $statusLabelShort=$langs->trans('packagingStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('packagingStatusRefused'); $statusLabelShort=$langs->trans('packagingStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('packagingStatusAccepted'); $statusLabelShort=$langs->trans('packagingStatusShortAccepted'); }
...
//class packagingDet extends SeedObject
//{
//    public $table_element = 'packagingdet';
//
//    public $element = 'packagingdet';
//
//
//    /**
//     * packagingDet constructor.
//     * @param DoliDB    $db    Database connector
//     */
//    public function __construct($db)
//    {
